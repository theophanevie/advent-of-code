...
def count_pulse(start_module: Module, target_module: Module, output_pulse: Pulse) -> int:
    count = 0
    wired_pulses = Queue()

    while True:
        wired_pulses.put(WiredPulse(None, start_module, Pulse.LOW))
        count += 1
        ...
        while not wired_pulses.empty():
            ...
            for dst in wp.dst.output:
                if pulse_response == output_pulse and dst.slug == target_module.slug:
                    return count
...
def main(input_file: str) -> int:
    ...
    counter = []
    for module in modules["broadcaster"].output:
        counter.append(count_pulse(module, modules["hj"], Pulse.HIGH))
    print(counter)
    return math.lcm(*counter)
